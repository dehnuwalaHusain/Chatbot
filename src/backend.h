...
void identify_function ( std::string processedQuery )
{
	...
	else if ( processedQuery.find ( "WIKI" ) != std::string::npos )	{
		processedQuery.erase ( processedQuery.begin(), processedQuery.begin()+6 );

		for ( int iter = 0; iter < processedQuery.length () ; iter++ )	{
			if ( processedQuery [ iter ] == ' ' )
				processedQuery [ iter ] = '_';
		}
		find_information ( processedQuery );
	}
}
